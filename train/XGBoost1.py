...
#global mean and global standard derivation for normalization
...
#transform str type column name into int index
def colIndex(col):
    if col == 'PCL':
        return 1
    if col == 'PK':
...
#return Xlist: training input, yList: training output,
# minList: min value in the column for each feature instance, maxList: max value in the column for each feature instance
# 90% of training phase date are used to train models
def featureSet(lab):
    XList = []
    yList = []
    maxList = []
...
#load training features
    for id in range(1, 7445):
        ...
        filled = pd.read_csv(filled_path) #prefilled training data
        truth = pd.read_csv(truth_path) #training data groundtruth
        ...
        for row in range(0, data_num):   #load features for each instance
            row_filled_front = row - 1   #add front 2 rows
            row_filled_after = row + 1
            #TODO add more rows features
            row_filled_front_front = row-2 #add later 2 rows
            row_filled_after_after = row+2

            if (row == 0): #check if front 2 rows are out of range
                row_filled_front = row_filled_after
                row_filled_front_front = row_filled_after

            if (row == data_num - 1): #check if later 2 rows are out of range
                row_filled_after = row_filled_front
                row_filled_after_after = row_filled_front

            ##TODO add more rows features
            if (row == 1): #check if front 1 row is out of range
                row_filled_front_front = row_filled_front

            if (row == data_num -2): #check if later 1 row is out of range
                row_filled_after_after = row_filled_after


            #only include rows in groundtruth that have no missing data into training instances
            if ((not math.isnan(truth.iloc[row]['PCL'])) and
                ...
                #load features column by column
                #normalization
                #TODO add more rows features
                ...
                #add time differences as a feature into the instance
                #transform time in seconds to time in minutes
                ...
                min_num = np.nanmin(truth[lab]) #min value in the column for each instance
                max_num = np.nanmax(truth[lab]) #max value in the column for each instance
...
#implement same function as featureSet(lab)
# 10% of training phase date are used to validate models
def loadTestData(lab):
    ...
    for id in range(7445, 8268): #extract validation dataset
...
#model learning
#input: training model inputs and training outputs, and corresponding min and max value
# input; validation model inputs and outputs, and corresponding min and max value
def train(X_train, y_train, X_test, y_test, max_test, min_test, max_small, min_small):
    ...
    def nRMSE(pred, y):
        ...
        if (len(pred) != len(max_num)): #evaluation matric for validation model
            max_num = np.array(max_small)
            min_num = np.array(min_small)
            result = np.mean(((pred - y.get_label()) ** 2) / (max_num - min_num))

        else: #evaluation matrix for training model
            result = np.mean(((pred - y.get_label()) ** 2) / (max_num - min_num))
...
#avoid overfitting
    ...
    other_params = {'learning_rate': 0.1, 'n_estimators': 1100, 'max_depth': 5, 'min_child_weight': 3}
