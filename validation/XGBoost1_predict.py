...
#take in every row in validation dataset
    for row in range(0, data_num):
        row_filled_front = row - 1 #load front 2 rows and later 2 rows
...
if __name__ == '__main__':
    ...
    model = []
    model1 = pickle.load(open("model1.pickle.dat", "rb")) #load model with best performance
    model.append(model1)


#predict missing value for 1 column
    ...
    for id in range(7445, 8268):
        ...
        for col in range(1, 2):  # to skip Charttime

            x_id_test = each_ID_testData(id, lab_name[col - 1]) #load model input list
            result = Test(model[0], x_id_test) #save model output list

            s = pd.Series(dataset[col])

#compare with labeled csv
#only evaluate masked missing value positions
...
np.save("output1", evaluate_result)
