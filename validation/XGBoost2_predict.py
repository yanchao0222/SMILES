...
def train(X_train, y_train, X_test, y_test, max_test, min_test, max_small, min_small):
    ...
    model = xgb.XGBRegressor(max_depth=6, learning_rate=0.1, n_estimators=500, silent=False, objective='reg:gamma')
    ...
    # save model to file
    pickle.dump(model, open("model2.pickle.dat", "wb"))
...
if __name__ == '__main__':
    ...
    model = []
    model2 = pickle.load(open("model2.pickle.dat", "rb"))  # load model with best performance
    model.append(model2)
    ...
    for id in range(7445, 8268):
        ...
        for col in range(2, 3):  # to skip Charttime
...
np.save("output2", evaluate_result)
